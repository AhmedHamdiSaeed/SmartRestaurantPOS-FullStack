...
foreach ($svc in $services) {
    ...
    Start-Process powershell -ArgumentList "-NoExit", "-Command",
        "Set-Location '$svcDir'; Write-Host '▶ $svcName' -ForegroundColor Green; ..\mvnw.cmd spring-boot:run"
...
Start-Process powershell -ArgumentList "-NoExit", "-Command",
    "Set-Location '$frontendDir'; Write-Host '▶ Angular Frontend' -ForegroundColor Green; npm start"
...
Write-Host ""
Write-Host '  NOTE: Gateway root http://localhost:8080 shows 404 - this is normal.' -ForegroundColor DarkGray
